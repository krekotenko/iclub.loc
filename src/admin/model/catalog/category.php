...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
        ...
        /////>>>>>Reviews
        $categoryAttributesQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_reviews WHERE category_id = '" . (int)$category_id . "'" );
        $categoryAttributes = array_map(function ($item) {
            return $item['id'];
        },$categoryAttributesQuery->rows);
        if (isset($data['category_attribute'])) {
            foreach ($data['category_attribute'] as $attribute) {
                if(isset($attribute['id'])) {
                    if(in_array($attribute['id'],$categoryAttributes)) {
                        $this->db->query("UPDATE " . DB_PREFIX . "category_reviews SET name = '" . $this->db->escape($attribute['name']) . "' WHERE id = '" . (int)$attribute['id'] . "'");
                    }
                    else {
                        $this->db->query("DELETE FROM " . DB_PREFIX . "category_reviews WHERE id = '" . (int)$attribute['id'] . "'");
                    }
                }
                else {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "category_reviews SET category_id = '" . (int)$category_id . "', name = '" . $this->db->escape($attribute['name']) . "'");
    ...
    public function getCategoryReviews($category_id = 0) {
        $query = $this->db->query("SELECT id,name FROM " . DB_PREFIX . "category_reviews WHERE category_id = '" . (int)$category_id . "'" );
        return $query->rows;
    }

    public function getCategoryReviewContentByreview($id = 0, $product_id, $review_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_reviews_value WHERE category_review_id = '" . (int)$id . "' AND product_id = '" . $product_id. "' AND review_id = '" . $review_id. "'" );
        return $query->rows;
    }
	...
	public function getAttributes($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_reviews WHERE category_id = '" . (int)$category_id . "'");
		return $query->rows;
	}
