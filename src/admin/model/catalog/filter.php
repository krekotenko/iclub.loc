...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET keyword = '".$filter['filter_keyword']."', query = 'filter_id=" . (int)$filter['filter_id'] . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
                    $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'filter_id=" . (int)$filter['filter_id'] . "'");
                    $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'filter_id=" . (int)$filter['filter_id'] . "', keyword = '".$filter['filter_keyword']."'");
	...
	public function deleteFilter($filter_group_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter` WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_description` WHERE filter_group_id = '" . (int)$filter_group_id . "'");
        $this->db->query("DELETE FROM `" . DB_PREFIX . "url_alias` WHERE query = 'filter_id=" . (int)$filter_group_id . "'");
	}
	...
	public function getFilter($filter_id) {
        ...
        $filter = $this->db->query("SELECT `keyword` FROM " . DB_PREFIX . "url_alias WHERE `query` = 'filter_id=".$filter_id."'");

        if($filter->num_rows > 0) $query->row['keyword'] = $filter->row['keyword'];
        else $query->row['keyword'] = '';
	...
	public function getFilterDescriptions($filter_group_id) {
		...
		foreach ($filter_query->rows as $filter) {
            ...
            $get_keyword = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'filter_id=" . (int)$filter['filter_id'] . "'");

            if($get_keyword->num_rows > 0) $keyword = $get_keyword->row['keyword'];
            else $keyword = '';
