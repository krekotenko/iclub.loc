...
class ControllerCatalogReview extends Controller {
	...
	protected function getForm() {
		...
		if($review_info && $review_info['product_id']) {
            ...
            $resultReviews = null;
            $productCategories = $this->model_catalog_product->getCategories($review_info['product_id']);
            ...
            if($resultReviews) {
                ...
                {
                    $reviewContent = $this->model_catalog_category->getCategoryReviewContentByreview($review['id'], $review_info['product_id'],$review_info['review_id']);
                    if(isset($reviewContent[0])) {
                        if(!isset($reviewContent[0]['value'])) {
                            continue;
                        }
                        $data['product_reviews'][] = [
                            'id' => $review['id'],
                            'name' => $review['name'],
                            'value' => $reviewContent[0]['value']
                        ];
                    }
