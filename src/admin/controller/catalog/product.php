...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
      ...
      $category =  $this->model_catalog_product->getProductCategories($result['product_id']);
		...
		$data['entry_price_proc'] = $this->language->get('entry_price_proc');
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['kursUsd'] = $this->config->get('config_usd');
		...
		if (isset($this->request->post['price_discount'])) {
			$data['price_discount'] = $this->request->post['price_discount'];
		} elseif (!empty($product_info)) {
			$data['price_discount'] = $product_info['price_discount'];
		} else {
			$data['price_discount'] = '';
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['upc']) > 0) {
			$upc_info = $this->model_catalog_product->getUpc($this->request->post['upc']);

			if ($upc_info && isset($this->request->get['product_id']) && $upc_info['product_id'] != $this->request->get['product_id']) {
				$this->error['upc'] = sprintf($this->language->get('error_upc'));
			}

			if ($upc_info && !isset($this->request->get['product_id'])) {
				$this->error['upc'] = sprintf($this->language->get('error_upc'));
			}
		}
