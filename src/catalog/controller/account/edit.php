	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (($this->customer->getEmail() != $this->request->post['email'])) {
                $this->model_account_customer->editCustomer($this->request->post, 0);
                $this->response->setOutput(json_encode(array('status'=>'change_mail')));
            } else {
		...
		if ($this->request->server['REQUEST_METHOD'] != 'POST') {
            ...
            if (isset($customer_info)) {
                $this->load->model('account/address');
                $data['address'] =  $this->model_account_address->getAddress($customer_info['address_id']);
            } else {
                $data['address'] = [];
            }
