...
class ModelAccountAddress extends Model {
	...
	public function editAddress($data) {
		$this->db->query("UPDATE " . DB_PREFIX . "address SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '', company = '', address_1 = '" . $this->db->escape($data['address']['address']) . "', address_2 = '', postcode = '', city = '" . $this->db->escape($data['address']['city']) . "', zone_id = '" . (int)$data['address']['zone_id'] . "', country_id = '220', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "' WHERE main  = 1 AND customer_id = '" . (int)$this->customer->getId() . "'");
		/*if (!empty($data['default'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET address_id = '" . (int)$address_id . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");
		}*/
	}
