	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
                ...
                $special =$product_special_query->num_rows ? $product_special_query->row['price'] : 0;
                $price_discount = $product_query->row['price_discount'];
                ...
                $product_data[] = array(
					...
					'price_discount'   => ($price_discount + $option_price),
	...
	public function getTotal() {
		...
		foreach ($this->getProducts() as $product) {
            if ($this->customer->isLogged() || ! $this->config->get('config_customer_price')) {
                $price = $product['price'] * $this->config->get('config_usd');
            } else {
                $price = false;
            }
            if ($product['price_discount'] && ($this->customer->isLogged() || ! $this->config->get('config_customer_price'))) {
                $price_discount = ($product['price'] - ($product['price_discount'] * $product['price']) / 100) * $this->config->get('config_usd');
            ...
            if ((float)$product['special']) {
                ...
                $specialPrice = (($product['price'] - (($specialDiscount * $product['price']) / 100)) * $this->config->get('config_usd'));
            } else {
                $specialPrice = $price_discount;
            }
			$total +=$specialPrice ? intval(str_replace(' ', '', $specialPrice)*$product['quantity']) : intval(str_replace(' ', '', $price)*$product['quantity']);
            $totalWithoutDiscount  +=intval(str_replace(' ', '', $price)*$product['quantity']);
		}

		return ['total' => $total, 'totalWithoutDiscount' => $totalWithoutDiscount];
	}
